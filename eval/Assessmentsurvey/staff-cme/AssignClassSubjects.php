...
<div class="alert alert-success" id="successAlert" role="alert">
                        ...
                        <form id="assignFacultyForm">
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="department" name="department" required>
                                    ...
                                    <?php 
                                    $departments = $conn->query("SELECT * FROM department_list WHERE name IN ('CME', 'CME - IRREGULAR') ORDER BY name ASC");
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="faculty" name="faculty_id" required>
                                    ...
                                    <?php 
                                    $faculty = $conn->query("SELECT id, CONCAT(firstname, ' ', lastname) as name FROM faculty_list WHERE department = 'CME' ORDER BY lastname ASC");
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="class" name="class_id" required>
                                    ...
                                    <?php 
                                    $classes = $conn->query("SELECT id, CONCAT(level, ' - ', section, ' (', curriculum, ')') as class_name FROM class_list WHERE department = 'CME' ORDER BY level ASC");
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="subject" name="subject_id" required>
                                    ...
                                    <?php 
                                    $subjects = $conn->query("SELECT id, CONCAT(code, ' - ', subject) as subject_name FROM subject_list WHERE department = 'CME' ORDER BY subject ASC");
   ...
   <script>
    ...
    function loadDepartmentTabs() {
        const tabsHtml = `
            <div class="department-tab active" data-department="CME">CME</div>
            <div class="department-tab" data-department="CME - IRREGULAR">CME - IRREGULAR</div>
        `;
        $('#departmentTabs').html(tabsHtml);
        
        // Activate CME tab by default
        $('.department-tab[data-department="CME"]').addClass('active');
    }
    ...
    function initializeDataTable(data) {
        ...
        assignmentsTable = $('#assignmentsTable').DataTable({
            ...
            buttons: [
                {
                    ...
                    title: 'CME Faculty Assignments',
                    exportOptions: {
                        columns: [0, 1, 2], // Only export Faculty, Class, and Subject columns
                        modifier: {
                            search: 'applied'
                        }
                    }
            ...
            language: {
                ...
                search: "",
                lengthMenu: "_MENU_ assignments per page",
    ...
    function loadAssignments(department = 'all') {
        $.ajax({
            url: 'ajax.php?action=load_assignments_faculties_cme',
            method: 'POST',
            data: { department: department },
            success: function(response) {
            ...
            success: function(response) {
                try {
                    ...
                    if (stats.status === 'success') {
                        // Update basic stats for CME department only
                        const cmeStats = stats.departmentStats['CME'];
                        if (cmeStats) {
                            $('#totalAssignments').text(cmeStats.assignments);
                            $('#totalFaculty').text(cmeStats.faculty);
                        } else {
                            $('#totalAssignments').text(0);
                            $('#totalFaculty').text(0);
                        }

                        // Update recent assignments for CME department only
                        if (stats.recentAssignments) {
                            let recentHtml = '<div class="recent-assignments">';
                            stats.recentAssignments.forEach(assignment => {
                                if (assignment.department === 'CME') {
                                    recentHtml += `
