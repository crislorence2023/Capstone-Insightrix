...
$class_query = "SELECT DISTINCT CONCAT(curriculum, ' ', level, '-', section) as class_name 
                FROM class_list 
...
if ($class_name) {
    $query .= " AND CONCAT(cl.curriculum, ' ', cl.level, '-', cl.section) = ?";
}
                    ...
                    <select class="form-select" name="class_name" id="class_name">
                        ...
                        <?php while($row = $class_result->fetch_assoc()): ?>
                            <option value="<?php echo $row['class_name']; ?>" 
                                    <?php echo ($class_name == $row['class_name']) ? 'selected' : ''; ?>>
...
$(document).ready(function() {
    ...
    function loadEvaluationStatus() {
        ...
        $.ajax({
            url: 'ajax.php?action=get_evaluation_status_cme',
            method: 'POST',
            data: filters,
            success: function(response) {
