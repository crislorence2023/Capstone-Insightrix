    ...
    <div class="card shadow-lg border-0 rounded-3">
        <div class="card-header bg-light bg-gradient text-white py-3">
            <div class="d-flex align-items-center">
                <h5 class="mb-0 flex-grow-1 text-bold">Add New CEAS Staff</h5>
                <button type="button" class="btn btn-outline-primary btn-sm" onclick="location.href = 'indexsuperadmin.php?page=ceas_staff'">
                    Back to List
                </button>
            </div>
        </div>
        <div class="card-body p-4">
            <form action="" id="manage_staff" class="needs-validation" novalidate>
                ...
                <div class="text-end mt-4 pt-3 border-top">
                    <button type="button" class="btn btn-light me-2" onclick="location.href = 'index.php?page=ceas_staff'">Cancel</button>
...
<script>
    ...
    $(document).ready(function() {
        // File input handling with ellipsis
        $('#avatar').on('change', function() {
            var fileName = $(this).val().split('\\').pop();
            var formText = $('.avatar-upload .form-text');
            if (fileName) {
                // Truncate filename if it's too long (e.g., over 20 characters)
                var displayName = fileName.length > 20 ? 
                    fileName.substring(0, 17) + '...' : fileName;
                formText.text('Selected: ' + displayName);
                formText.attr('title', 'Selected: ' + fileName); // Full filename in tooltip
            } else {
        ...
        $('#manage_staff').submit(function(e) {
            ...
            if (this.checkValidity()) {
                $.ajax({
                    url: 'ajax.php?action=save_staff_ceas',
                    ...
                    success: function(resp) {
                        if(resp == 1) {
                            ...
                            setTimeout(function() {
                                location.href = 'index.php?page=ceas_staff';
                            }, 750);
                        } else if(resp == 2) {
