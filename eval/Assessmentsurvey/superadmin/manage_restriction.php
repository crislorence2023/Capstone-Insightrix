		...
		<div class="row">
			<div class="col-md-4 border-right">
				...
				<div class="form-group">
					...
					<select name="" id="subject_id" class="form-control form-control-sm select2">
						<option value=""></option>
						<?php 
						$subject = $conn->query("SELECT id,concat(code,' - ',subject,' (',department,')') as subj FROM subject_list");
						$s_arr = array();
			...
			<div class="col-md-8">
				<table class="table table-condensed" id="r-list">
					...
					<tbody>
						<?php 
						$restriction = $conn->query("SELECT r.*, s.code, s.subject, s.department FROM restriction_list r LEFT JOIN subject_list s ON r.subject_id = s.id WHERE r.academic_id = {$_GET['id']} ORDER BY r.id ASC");
						...
						<tr>
							...
							<td>
								<b><?php echo $row['code'].' - '.$row['subject'].' ('.$row['department'].')' ?></b>
...
<script>
    ...
    // Add loading indicator functions if not already present
    function start_load(){
