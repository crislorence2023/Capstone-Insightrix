...
/* Base responsive layout */
  ...
  .mobile-subject-dropdown select {
    width: 100%;
    padding: 0.5rem;
    font-size: 0.875rem;
    border-radius: 0.25rem;
...
@media screen and (max-width: 768px) {
  ...
  /* Historical performance text adjustments */
  .historical {
    font-size: 16px !important;
  ...
  .card.border-info.shadow-sm .container {
    padding: 1rem !important;
  }

  /* Ensure proper margin between charts */
  .card.border-info.shadow-sm:not(:last-child) {
    margin-bottom: 2rem;
  }
        ...
        scales: {
            ...
            x: {
                ...
                ticks: {
                    ...
                    font: {
                        size: 14 // Larger font size for modal
                    }
                }
            },
            y: {
...
function handleChartClick(event) {
    ...
    const sourceChart = event.target.chart;
    
    // Show modal
    modal.classList.add('show');
    document.body.style.overflow = 'hidden';
    
    // Create modal chart if it doesn't exist
    if (!modalChart) {
        modalChart = createModalChart(sourceChart);
    } else {
        // Update existing modal chart
        modalChart.config.data = JSON.parse(JSON.stringify(sourceChart.config.data));
        modalChart.update();
    }
}
...
function initializeChartInteractions() {
    ...
    document.querySelectorAll('#historicalChart, #performanceDistChart').forEach(canvas => {
...
document.addEventListener('DOMContentLoaded', initializeChartInteractions);
