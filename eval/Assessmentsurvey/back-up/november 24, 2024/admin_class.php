...
Class Action {
	...
	function login4() {
		...
		if (!isset($college) || empty($college)) {
			return 4; // Return new code for missing college selection
		}
		...
		if($qry->num_rows > 0) {
			...
			// Check if department matches selected college
			if ($row['department'] != $college) {
				return 5; // Return new code for department mismatch
			}
			...
			if ($password_verified) {
				...
				$_SESSION['user_id'] = $row['id'];
				$_SESSION['login_department'] = $college; // Store selected department
				...
				// Return department-specific code for redirection
				switch($college) {
					case 'COE':
						return 'COE';
					case 'CEAS':
	...
	function update_user(){
		try {
			...
			if($save){
				...
				foreach ($_POST as $key => $value) {
					if($key != 'password' && !is_numeric($key) && !empty($value)) {
						$_SESSION['login_'.$key] = $value;
					}
				}
				if(isset($_FILES['img']) && !empty($_FILES['img']['tmp_name'])) {
...
function save_restriction() {
    ...
    AND e.academic_id = " . intval($sem['id']);
		...
		function get_notifications() {
			...
			$notifications = array();
			while($row = $result->fetch_assoc()) {
				// Add notification type-specific data
				if($row['notification_type'] == 'low_rating') {
					$row['rating_color'] = $row['rating_value'] < 1.5 ? 'danger' : 'warning';
				}
				$notifications[] = $row;
			}
