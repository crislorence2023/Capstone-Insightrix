...
Class Action {
	...
	function login4() {
		...
		$qry = $this->db->query("SELECT * FROM staff WHERE email = '".$identifier."'");
		...
		if($qry->num_rows > 0) {
			...
			if(password_verify($password, $row['password'])) {
				...
				if($row['is_password_changed'] == 0) {
					// Store minimal info needed for password change
					$_SESSION['temp_staff_id'] = $row['id'];
					$_SESSION['temp_staff_email'] = $row['email'];
					return 3; // Redirect to change password
				}
				...
				$_SESSION['login_id'] = $row['id'];
				$_SESSION['login_type'] = 3; // Type 3 for staff
				$_SESSION['login_name'] = $row['firstname'] . ' ' . $row['lastname'];
				$_SESSION['login_avatar'] = $row['avatar'];
				$_SESSION['login_view_folder'] = 'admin/'; // Staff folder
				
				// Check and store verification status if needed
				if(!empty($row['verification_code']) && !empty($row['verification_code_expiry'])) {
				...
				if(isset($_POST['remember']) && $_POST['remember'] == 'on') {
					...
					setcookie('staff_remember_token', $token, time() + (86400 * 30), "/"); // 30 days
					// Store token in database (you'll need to add a remember_token column)
					$this->db->query("UPDATE staff SET remember_token = '$token' WHERE id = ".$row['id']);
				}
...
function change_password(){
    ...
    function forgot_password() {
		...
		function forgot_password() {
			...
			// Validate inputs
			if(!isset($email) || !isset($login) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
			...
			$qry = $this->db->query("SELECT * FROM $table WHERE email = ?");
			...
			if($result->num_rows > 0) {
				...
				$update = $this->db->query("UPDATE $table SET 
					...
					WHERE id = ?");
				$update->bind_param("ssi", $verification_code, $expiry, $row['id']);
				...
				return 3; // Error in update or email sending
			}
			return 2; // Email not found
		}
	}
