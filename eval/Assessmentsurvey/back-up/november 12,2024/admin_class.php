...
// Inside your crud class
		...
		public function check_assignment() {
			...
			try {
				...
				$stmt = $this->db->prepare("SELECT id FROM restriction_list WHERE faculty_id = ? AND class_id = ? AND subject_id = ?");
				$stmt->bind_param("iii", $faculty_id, $class_id, $subject_id);
				$stmt->execute();
				$result = $stmt->get_result();
		...
		public function save_assignment() {
			...
			try {
				...
				$insert_stmt = $this->db->prepare("INSERT INTO restriction_list (faculty_id, class_id, subject_id) VALUES (?, ?, ?)");
				$insert_stmt->bind_param("iii", $faculty_id, $class_id, $subject_id);
				
				if ($insert_stmt->execute()) {
					return json_encode(['status' => 'success', 'message' => 'Assignment saved successfully']);
				} else {
					return json_encode(['status' => 'error', 'message' => 'Failed to save assignment']);
				}
...
public function load_assignments_grid() {
    try {
        ...
        LEFT JOIN restriction_list r ON r.class_id = c.id AND r.subject_id = s.id
        LEFT JOIN faculty_list f ON f.id = r.faculty_id
        WHERE 1=1";
