	...
	function login4() {
		...
		if($qry->num_rows > 0) {
			...
			if ($password_verified) {
				...
				$_SESSION['login_id'] = $row['id'];
				$_SESSION['login_type'] = 1; // Hardcoded for superadmin
				$_SESSION['login_view_folder'] = 'admin/'; // Hardcoded admin folder
				$_SESSION['user_id'] = $row['id'];
 ...
 //06-10-2024
	...
	function save_evaluation() {
		try {
			...
			if (!isset($_SESSION['login_id']) || !isset($_SESSION['academic']['id'])) {
				return json_encode(['status' => 'error', 'message' => 'Session data missing']);
			}
	
			// Extract and sanitize POST data
			$required_fields = ['academic_id', 'subject_id', 'class_id', 'restriction_id', 'faculty_id', 'qid', 'rate'];
			...
			// Prepare the SQL query for evaluation_list
			...
			if (!$save) {
				throw new Exception("Failed to save evaluation: " . $this->db->error);
			}
			...
			foreach ($qid as $q_id) {
				if (!isset($rate[$q_id])) {
					continue; // Skip if no rating provided for this question
				}
				...
				$answer_sql = "INSERT INTO evaluation_answers 
							  (evaluation_id, question_id, rate) 
			...
			if (!$success) {
				// Rollback if answers insertion failed
				$this->db->query("DELETE FROM evaluation_list WHERE evaluation_id = $evaluation_id");
				throw new Exception("Failed to save evaluation answers: " . $this->db->error);
			}
	
			// Check for low ratings after successful save
			$this->check_low_ratings();
	
			return json_encode(['status' => 'success', 'message' => 'Evaluation saved successfully']);
    ...
    AND e.academic_id = " . intval($sem['id']);
		...
		function create_notification($faculty_id, $message) {
			$query = "INSERT INTO notifications (faculty_id, message) VALUES (?, ?)";
			$stmt = $this->db->prepare($query);
		...
		function get_notifications() {
			extract($_POST);
			$query = "SELECT * FROM notifications WHERE faculty_id = ? AND is_read = 0 ORDER BY created_at DESC";
			$stmt = $this->db->prepare($query);
	...
	function check_low_ratings() {
		$faculty_id = $_POST['faculty_id'];
		$subject_id = $_POST['subject_id'];
		$class_id = $_POST['class_id'];
		
		// Get current average rating
		$average_rating = $this->calculate_subject_rating($faculty_id, $subject_id, $class_id);
		
		if ($average_rating < 2) {
			// Get subject and class details
			$query = "SELECT s.code, s.subject as name, cl.level as class_name
					 FROM subject_list s 
					 JOIN class_list cl ON cl.id = ?
					 WHERE s.id = ?";
			...
			// Check if notification already exists for this rating
			$exists = $this->check_existing_notification($faculty_id, $subject_info['code'], $average_rating);
			
			if (!$exists) {
				$message = "Your subject {$subject_info['code']} - {$subject_info['name']} for {$subject_info['class_name']} " .
						  "has received an average rating of " . number_format($average_rating, 2);
				
				$this->create_notification($faculty_id, $message);
			}
		}
	}
	...
	function check_existing_notification($faculty_id, $subject_code, $rating) {
		...
		function check_and_create_low_rating_notifications() {
			$query = "SELECT 
					  ...
					  JOIN restriction_list r ON e.id = r.evaluation_id
					  WHERE e.academic_id = ?
					  GROUP BY e.faculty_id, s.id, cl.id
					  HAVING average_rating < 2";
			...
			while($row = $result->fetch_assoc()) {
				$message = "Your subject {$row['code']} - {$row['name']} for {$row['class_name']} has received an average rating of {$row['average_rating']} from {$row['total_evaluations']} evaluations.";
