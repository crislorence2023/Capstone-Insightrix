		...
		public function get_dashboard_stats() {
			try {
				...
				$assignments_query = $this->db->prepare(
					"SELECT COUNT(*) as total FROM faculty_assignments 
					 WHERE academic_year_id = ? AND semester = ? AND is_active = 1"
				);
				...
				$faculty_query = $this->db->query("SELECT COUNT(*) as total FROM faculty_list WHERE status = 1");
				...
				while($row = $dept_result->fetch_assoc()) {
					$department_stats[$row['department']] = $row['count'];
				}
