	...
	function update_staff() {
		...
		public function get_class_subjects() {
			try {
				...
				// First, get the department and other class details
				$class_stmt = $this->conn->prepare("
					SELECT department, curriculum, level, section 
					FROM class_list 
					WHERE id = ?
				");
				...
				if (!$class_stmt->execute()) {
					throw new Exception("Error getting class details: " . $class_stmt->error);
				}
				...
				// Get current academic year
				$academic_stmt = $this->conn->prepare("
					SELECT id 
					FROM academic_list 
					WHERE status = 1 
					ORDER BY id DESC 
				...
				$academic_id = $academic_row ? $academic_row['id'] : null;
				
				if (!$academic_id) {
					throw new Exception("No active academic year found");
				}
				
				// Get subjects with instructor and schedule information
				$stmt = $this->conn->prepare("
					...
					LEFT JOIN instructor_list i ON cs.instructor_id = i.id
					WHERE s.department = ?
					ORDER BY s.code ASC
				");
				
				$stmt->bind_param("iis", $class_id, $academic_id, $department);
				...
				while ($row = $result->fetch_assoc()) {
					$subjects[] = array(
						...
						'instructor_firstname' => htmlspecialchars($row['instructor_firstname'] ?? ''),
						'instructor_lastname' => htmlspecialchars($row['instructor_lastname'] ?? '')
					);
				}
				...
				echo json_encode(array(
					'status' => 'error',
					'message' => 'Failed to load subjects data: ' . $e->getMessage(),
					'data' => array()
				));
				exit;
			} finally {
