...
Class Action {
	...
	function login4() {
		extract($_POST);
		$type = array("", "users", "faculty_list", "student_list", "staff");
		$type2 = array("", "admin", "faculty", "student", "staff");
		...
		// Validate that only COT is allowed
		if ($college !== 'COT') {
		...
		$qry = $this->db->query("SELECT *, concat(firstname,' ',lastname) as name FROM {$type[4]} WHERE {$condition}");
		
		if($qry->num_rows > 0) {
			$row = $qry->fetch_array();
			$hashed_password = $row['password'];
			
			// Check if department matches COT
			if ($row['department'] != 'COT') {
				return 5; // Return code for department mismatch
			}
			...
			if ($password_verified) {
				...
				if ($update_hash) {
					$new_hash = password_hash($password, PASSWORD_DEFAULT);
					$update_query = "UPDATE {$type[4]} SET password = '{$new_hash}' WHERE id = {$row['id']}";
					$this->db->query($update_query);
				}
				...
				$_SESSION['login_view_folder'] = $type2[4].'/';
				$_SESSION['user_id'] = $row['id'];
				$_SESSION['login_department'] = 'COT'; // Always set to COT
				...
				return 'COT'; // Only return COT as valid response
			}
			return '2'; // Wrong password
		}
		return 2; // User not found
		...
		public function save_assignment_faculty() {
			try {
				...
				$check_query = $this->db->prepare(
					"SELECT id, is_active FROM faculty_assignments 
					WHERE faculty_id = ? AND class_id = ? AND subject_id = ? 
					AND academic_year_id = ? AND semester = ?"
				);
				
				$check_query->bind_param("iiiii", 
					$faculty_id, 
					$class_id, 
					$subject_id, 
					$academic_year_id, 
				...
				if($result->num_rows > 0) {
					...
					if($existing['is_active'] == 0) {
						...
						$update->bind_param("ii", $assigned_by, $existing['id']);
				...
				// If no existing assignment found, create new one
				$stmt = $this->db->prepare(
					"INSERT INTO faculty_assignments 
					(faculty_id, class_id, subject_id, academic_year_id, semester, assigned_by, is_active) 
					VALUES (?, ?, ?, ?, ?, ?, 1)"
				);
...
function login4_coe() {
    ...
    // Validate that only COE is allowed
    if ($college !== 'COE') {
        return 6; // Invalid department selection
    }
    ...
    // Check if the identifier is an email
    if (filter_var($identifier, FILTER_VALIDATE_EMAIL)) {
        $qry = $this->db->query("SELECT * FROM coe_staff WHERE email = '".$identifier."'");
    } else {
        return 2; // Invalid identifier (must be email for staff)
    }
...
function login4_cme() {
    ...
    // Validate that only CME is allowed
    if ($college !== 'CME') {
        return 6; // Invalid department selection
    }
    ...
    // Check if the identifier is an email
    if (filter_var($identifier, FILTER_VALIDATE_EMAIL)) {
        $qry = $this->db->query("SELECT * FROM cme_staff WHERE email = '".$identifier."'");
    } else {
        return 2; // Invalid identifier (must be email for staff)
    }
