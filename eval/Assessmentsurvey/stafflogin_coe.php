...
<body>
    <div class="login-container">
        <h1>Insightrix | <span style="color: teal;">LOGIN staff-COE<span></h1>
        <form id="login-form">
            <div class="email-container">
                <input type="email" id="identifier" name="identifier" placeholder="Email" required>
            </div>
    ...
    <script>
        $(document).ready(function(){
            $('#login-form').submit(function(e){
                e.preventDefault();
                $.ajax({
                    url:'ajax.php?action=login4_coe',
                    method:'POST',
                    data:$(this).serialize(),
                    error:err=>{
                        console.log(err)
                    ...
                    success:function(resp){
                        if(resp == 'COE'){
                            $.ajax({
                                url:'ajax.php?action=set_role',
                                method:'POST',
                                data:{role: 'staff_coe'},
                                success:function(){
                                    location.href = 'staff_coe_index.php';
                                }
                            });
                        } else {
