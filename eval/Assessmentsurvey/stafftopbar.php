...
document.addEventListener('DOMContentLoaded', function() {
    ...
    $('#logout_btn').click(function(e) {
        ...
        if(confirm("Are you sure you want to logout?")) {
            $(this).html('<i class="ri-loader-4-line fa-spin"></i> Logging out...');
            $(this).prop('disabled', true);
            
            $.ajax({
                url: 'ajax.php?action=logout4',
                method: 'POST',
                success: function(response) {
                    // Clear browser history
                    window.history.pushState(null, '', 'stafflogin.php?logout=1');
                    window.history.replaceState(null, '', 'stafflogin.php?logout=1');
                    
                    // Redirect with cache-busting parameter
                    window.location.href = 'stafflogin.php?logout=1&nocache=' + new Date().getTime();
                },
                error: function(xhr, status, error) {
                    console.error('Logout failed:', error);
                    alert('Failed to logout. Please try again.');
                    $('#logout_btn').html('<i class="ri-logout-box-line logout-icon"></i> <span class="logout-text">Logout</span>');
